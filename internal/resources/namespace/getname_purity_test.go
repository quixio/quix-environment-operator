...
func TestGetNamespaceNameIsPure(t *testing.T) {
	m := &DefaultManager{config: &config.OperatorConfig{NamespaceSuffix: "-qenv"}}
	env := &v1.Environment{Spec: v1.EnvironmentSpec{Id: "abc"}}

	if got := m.GetNamespaceName(env); got != "abc-qenv" {
		t.Fatalf("default name = %q, want %q", got, "abc-qenv")
	}
